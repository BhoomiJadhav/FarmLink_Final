...
export default function HeroSection() {
  ...
  const stats = [
    { label: t("hero.stats.farmers"), value: "10K+" },
    { label: t("hero.stats.contracts"), value: "500+" },
    { label: t("hero.stats.languages"), value: "15+" },
  ];
    ...
    >
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-12 px-4 py-20 lg:flex-row lg:items-center lg:py-28">
        ...
        <div className="max-w-xl space-y-8">
          ...
          <div className="flex flex-wrap gap-4">
            <Link to="/register">
              <button className="rounded-full bg-emerald-600 px-7 py-3 text-sm font-semibold text-white shadow-lg hover:bg-emerald-700">
                {t("hero.start")}
              </button>
            </Link>

            <a href="#how-it-works">
              <button className="rounded-full border border-emerald-300 bg-white/70 px-7 py-3 text-sm font-semibold text-emerald-800">
                {t("hero.learn")}
              </button>
            </a>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {stats.map((item) => (
              <div
                key={item.label}
                className="rounded-2xl bg-white/80 px-4 py-3 text-center shadow-sm"
              >
                <div className="text-xl font-bold text-emerald-700">
                  {item.value}
                </div>
        ...
        {/* RIGHT */}
        <div className="relative flex-1">
          <div className="relative mx-auto max-w-md rounded-[32px] bg-emerald-900/90 p-4 shadow-2xl">
            <div className="relative overflow-hidden rounded-3xl bg-black/40">
              <img
                src="/hero.webp"
                alt="FarmLink dashboard"
          ...
          {/* floating card 1 */}
          <div className="absolute -left-6 top-8 hidden w-56 rounded-2xl bg-white/90 p-4 text-sm shadow-lg md:block">
            <p className="text-xs font-semibold text-emerald-700">
              {t("hero.card1.title")}
            </p>
            <p className="mt-1 text-xs text-emerald-900/80">
          ...
          {/* floating card 2 */}
          <div className="absolute -right-4 bottom-8 hidden w-56 rounded-2xl bg-white/90 p-4 text-sm shadow-lg md:block">
            <p className="text-xs font-semibold text-emerald-700">
              {t("hero.card2.title")}
            </p>
            <p className="mt-1 text-xs text-emerald-900/80">
