        ...
        <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="mt-2">
              <h1 className="text-3xl md:text-[38px] font-bold text-[#064e3b] font-serif tracking-tight">
                {isFarmer
                  ? t("harvestTrackingFarmer")
                  : t("harvestTrackingBuyer")}
              </h1>
              <p className="text-[15px] text-slate-500 mt-2 font-medium italic">
                {t("harvestTrackingDesc")}
              </p>
            </div>
...
function InfoBox({ label, value, highlight }) {
  return (
    <div
      className={`rounded-2xl px-5 py-4 border transition-all ${highlight ? "bg-emerald-50/50 border-emerald-100 shadow-sm" : "bg-slate-50/50 border-slate-100"}`}
    >
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
        {label}
      </p>
...
function StatusBadge({ status, t }) {
  ...
  return (
    <span
      className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${map[safeStatus] || map.UNKNOWN}`}
    >
      {t(safeStatus.toLowerCase()) || safeStatus.replace(/_/g, " ")}
    </span>
  );
}
